...
async function fetchDramaDetail(bookId: string): Promise<DramaDetailResponse> {
  const response = await fetch(`${PROXY_URL}?endpoint=detail&params=bookId=${bookId}`);
  if (!response.ok) {
...
async function fetchAllEpisodes(bookId: string): Promise<Episode[]> {
  const response = await fetch(`${PROXY_URL}?endpoint=allepisode&params=bookId=${bookId}`);
  if (!response.ok) {
